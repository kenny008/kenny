...
$query1="SELECT email FROM trade";
	...
	if($go!=false){
	$query="INSERT INTO trade(asignal,fname,email,phone,tpl,password,dreg) VALUES('{$ram}','{$yrn}','{$yrn2}','{$yrn3}','{$yrn4}','{$yrn6}','{$dater}')";
	$result=mysql_query($query,$connection);
	if(mysql_affected_rows()==1){
	$to="support@meggaprofits.com,love4nation2013@gmail.com";
	$subject="Meggaprofits Account Management";
	$header="FROM: REGISTERATION(NOT ACTIVATED)\n";
	$message="REGISTRATION DETAILS(NOT ACTIVATED)"."\n"."\n";
	$message.="ACCOUNT SIGNAL: ".$ram."\n"."\n";
	$message.="FULL NAME: ".$yrn."\n"."\n";
	$message.="EMAIL ADDRESS: ".$yrn2."\n"."\n";
	$message.="PHONE NUMBER: ".$yrn3."\n"."\n";
	$message.="TRADING PLATFORM TYPE: ".$yrn4."\n"."\n";
	$message.="PASSWORD: ".$yrn6."\n"."\n";
	if(mail($to,$subject,$header,$message)){
		
		$to= $yrn2;
	$subject="Meggaprofits";
	$header="FROM:Account Management Confirmation\n";
	$message="Dear ".$yrn."\n"."\n";
	...
	$message.="http://meggaprofits.com/sts2.php?con=".urlencode($yrn2)."\n"."\n";
	...
	if(mail($to,$subject,$header,$message)){
header("Location:adib2.php?em=$yrn2");
}
	else{
...
<?php require_once("includes/hamans.php");?>
